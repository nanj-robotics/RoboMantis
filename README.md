...
| Joint Motors | Robstride Dynamics: 2× RS06, 2× RS03, 10× RS00 (one set per arm) |
...
├── robomantis_hardware/         # ros2_control hw, Robstride Dynamics CAN driver,
│                                # S-curve generator, zero-torque ctrl for both arms (C++)
...
- **Hand-eye calibration**: Must be performed once for your specific camera mounting
  position using `easy_handeye2` before grasping can work. With a fixed camera and
  dual arms, calibrate each arm's base_link relative to the camera frame.
