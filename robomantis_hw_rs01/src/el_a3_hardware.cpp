...
bool RsA3HardwareInterface::parseJointConfig(const hardware_interface::HardwareInfo& info)
{
  ...
  for (const auto& joint : info.joints) {
    ...
    if (joint.parameters.count("motor_type")) {
      ...
      } else if (type_str == "RS01") {
        config.motor_type = MotorType::RS01; 
...
{
  // 【双臂修复】start/stop_interfaces 是 controller_manager 下发的【全局】接口列表，
  // 会同时传给每个 hardware。只响应属于本硬件自身关节的接口，否则单切另一条臂到
  // effort(零力矩) 时，本硬件也会被误切到 effort 模式而瘫掉。
  auto own_interface = [this](const std::string & iface) {
    for (const auto & jc : joint_configs_) {
      if (iface.rfind(jc.name + "/", 0) == 0) return true;  // 以 "<本关节名>/" 开头
    }
    return false;
  };
  bool wants_effort = false;
  for (const auto& iface : start_interfaces) {
    if (!own_interface(iface)) continue;
  ...
  const std::vector<std::string>& stop_interfaces)
{
  ...
  // 【双臂修复】只统计属于本硬件自身关节的接口（全局列表里可能含另一条臂）
  auto own_iface = [this](const std::string & iface) {
    for (const auto & jc : joint_configs_) {
      if (iface.rfind(jc.name + "/", 0) == 0) return true;
    }
    return false;
  };
  for (const auto& iface : start_interfaces) {
    if (!own_iface(iface)) continue;
    if (iface.find("effort") != std::string::npos) {
  ...
  const rclcpp::Time& /*time*/, const rclcpp::Duration& period)
{
  ...
  for (size_t i = 0; i < joint_configs_.size(); ++i) {
    ...
    if (effort_mode_) {
      ...
      motor_kp = std::clamp(joint_kp, 0.0, params.kp_max);
      motor_kd = std::clamp(joint_kd, 0.0, params.kd_max);
